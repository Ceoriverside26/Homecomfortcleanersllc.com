...
async function signatureFor(url: string, body: string, key: string) {
...
async function validSignature(req: Request, body: string) {
  ...
  if (!signature || !key) {
    console.error('square-webhook-auth-missing', {
  ...
  // Square signs the exact notification URL + raw request body.
  // Prefer the actual request URL so a typo/whitespace in an env URL cannot break verification.
  const urls = [req.url]
  const configuredUrl = Deno.env.get('SQUARE_WEBHOOK_URL')?.trim()
  if (configuredUrl && configuredUrl !== req.url) urls.push(configuredUrl)

  for (const url of urls) {
    const expected = await signatureFor(url, body, key)
    if (expected === signature) return true
  }

  console.error('square-webhook-invalid-signature', {
    requestUrl: req.url,
    configuredUrlMatchesRequest: configuredUrl === req.url,
    hasSignature: true,
...
Deno.serve(async req => {
  try {
    ...
    if (!['booking.created', 'booking.updated'].includes(event.type)) {
    ...
    if (booking.customer_id) {
      try {
        ...
        const name = [c.customer?.given_name, c.customer?.family_name]
          .filter(Boolean)
          .join(' ')
    ...
    const row = {
      ...
      assigned_email:
        Deno.env.get('SQUARE_DEFAULT_ASSIGNEE_EMAIL') ||
        'ceo@riversideluxeretreats.com',
      assigned_name:
        Deno.env.get('SQUARE_DEFAULT_ASSIGNEE_NAME') || 'Philip Brooks',
      status: canceled ? 'canceled' : completed ? 'completed' : 'assigned',
    }

    const { error } = await supabase
      .from('jobs')
      .upsert(row, { onConflict: 'square_booking_id' })
    if (error) throw error

    console.log('square-booking-synced', { bookingId: booking.id, eventType: event.type })
